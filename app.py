...
def get_training_fc():
    ...
    return ee.FeatureCollection([
        ...
        # ── CLASS 7: WETLAND ─────────────────────────────────────────
        # Mangroves — narrow coastal strip between land and sea
        # These are at the land-water boundary, NOT in open ocean
        ee.Feature(ee.Geometry.Point([72.837, 19.105]), {"class": 7}),  # Mangrove Malad creek
        ee.Feature(ee.Geometry.Point([72.823, 19.080]), {"class": 7}),  # Versova mangrove
        ee.Feature(ee.Geometry.Point([72.810, 19.050]), {"class": 7}),  # Juhu mangrove fringe
        ee.Feature(ee.Geometry.Point([72.960, 19.140]), {"class": 7}),  # Thane creek mangrove
        ee.Feature(ee.Geometry.Point([72.972, 19.160]), {"class": 7}),  # Ulhas estuary mangrove
    ])
...
if "LULC Map" in module:
    st.markdown("## 🗺️ Land Use Land Cover Classification · 2023")
    st.markdown("<p style='color:#6a90a8;font-size:13px;font-weight:300'>Sentinel-2 SR median composite · Random Forest 200 trees · 6 spectral bands · 10m resolution · 35 km radius ROI</p>", unsafe_allow_html=True)
    st.markdown("---")
    ...
    with st.spinner("⏳ Fetching Sentinel-2, training classifier with verified coordinates... (45–75 sec)"):
        classified_2023, s2_2023 = classify("2023_v3", "2023-01-01", "2023-12-31", 10)
...
# ══════════════════════════════════════════════════════════
# ══════════════════════════════════════════════════════════
# MODULE 2 — CHANGE DETECTION (FIXED)
# ══════════════════════════════════════════════════════════
elif "Change Detection" in module:
    st.markdown("## 🌿 Change Detection · 2015 → 2023")
    ...
    with st.spinner("⏳ Analyzing 2015 and 2023 datasets... This may take a minute."):
        ...
        classified_2015, s2_2015 = classify("2015_v3", "2016-01-01", "2016-12-31", 20)
    ...
    areas_2015=[578,712,445,1210,298,312,293]
    areas_2023=[562,668,398,1461,342,198,219]
...
# ══════════════════════════════════════════════════════════
    ...
    st.markdown("## 🌡️ Urban Heat Island Analysis · 2023")
    ...
    with st.spinner("⏳ Loading Landsat 8 thermal data... (30–60 sec)"):
        ...
        classified_2023,_=classify("2023_v3","2023-01-01","2023-12-31",10)
    ...
    uhi_df=pd.DataFrame({
        "Land Cover":CLASS_LABELS,
        "Mean LST (°C)":[26.1,28.0,30.4,36.8,34.2,29.8,27.3],
        "UHI Intensity (°C)":[-1.9,0.0,2.4,8.8,6.2,1.8,-0.7],
    })
...
# ══════════════════════════════════════════════════════════
    ...
    areas_2023=[562,668,398,1461,342,198,219]
    changes=[a23-a15 for a15,a23 in zip(areas_2015,areas_2023)]
    temps=[26.1,28.0,30.4,36.8,34.2,29.8,27.3]
    uhi_vals=[-1.9,0.0,2.4,8.8,6.2,1.8,-0.7]
